...
export function buildRequestEmbed({ badge, name, startDate, endDate, reason, submittedBy, userId, avatarUrl }) {
  return new EmbedBuilder()
    ...
    .addFields(
      { name: '<:Kick:1529200674783629433> Kick User  ',   value: '```' + badge     + '```', inline: true  },
...
export function applyExpired(embed) {
  const fields = (embed.fields ?? []).map(f =>
    (f.name.includes('الحالة') || f.name.includes('Status'))
      ? { ...f, value: '**' + STATUS.EXPIRED + '**' }
      : { ...f }
  );

  if (!fields.some(f => f.name.includes('Expired at') || f.name.includes('انتهت في'))) {
    fields.push({
      name: '⏰  انتهت في  |  Expired at',
  ...
  return EmbedBuilder.from(embed)
    ...
    .setTitle('⏰  انتهت الأجازة  |  Leave Expired')
    .setFields(fields);
}

export function buildExpiredUserPanel(embed) {
  return applyExpired(embed).setTitle('⏰  انتهت الأجازة  |  Your Leave Has Expired');
}
