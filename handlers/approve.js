...
export async function handleApprove(interaction) {
  ...
  addVacation({ ownerId, name, badge, startDate, endDate, requestMsgId: message.id });
  incApproved();

  // ── Schedule reminders (only if dates are valid range) ────────
  if (validateDateRange(startDate, endDate)) {
    scheduleReminder({ requestMsgId: message.id, ownerId, startDateStr: startDate, endDateStr: endDate, client: guild.client });
  } else {
