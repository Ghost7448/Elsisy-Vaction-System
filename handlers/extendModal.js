...
export async function handleExtendModal(interaction) {
  ...
  const endField = panelEmbed?.fields?.find(f => f.name.includes('تاريخ الانتهاء') || f.name.includes('End Date') || f.name.includes('الانتهاء'));
  const startField = panelEmbed?.fields?.find(f => f.name.includes('تاريخ البداية') || f.name.includes('Start Date') || f.name.includes('البداية'));
  const currentEndDate = endField?.value?.replace(/```/g, '').replace(/`/g, '').trim() || '—';
  const startDate      = startField?.value?.replace(/```/g, '').replace(/`/g, '').trim() || '—';
  ...
  scheduleReminder({ requestMsgId: reminderKey, ownerId, startDateStr: startDate, endDateStr: newEndDate, client: guild.client, guildId: guild.id, requestChannelId: guild.channels.cache.get(process.env.REQUESTS_CHANNEL_ID)?.id || process.env.REQUESTS_CHANNEL_ID });
